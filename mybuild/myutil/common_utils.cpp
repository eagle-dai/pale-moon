...
#ifdef _WIN32
    FILETIME ft;
    GetSystemTimeAsFileTime(&ft); // unit 100ns
    uint64_t t64 = (static_cast<uint64_t>(ft.dwHighDateTime) << 32) | ft.dwLowDateTime;
    return t64 / 10000;
#else
    struct timeval tv;
...
bool ReadAllDataFromFile(const std::string& pathname, std::vector<unsigned char> &data)
{
    ...
    ifstream in(pathname.c_str(), ios::in | ios::binary);
    if (!in.good()) {
        return false;
    }
    data.resize(file_size);
...
{
    ofstream out(pathname, ios::binary);
    if (!out.good()) {
        return false;
    }
